...
const Customers = () => {
  ...
  return (
	<Layout title="Your customers" description="" classNameName="container">
		...
		{user.filter((v)=> v.name.toLowerCase().includes(value.toLowerCase()))
						.map((u)=>{ return (
									...
									<div className="col-10 col-md-4 mt-5" >
									  <div className="card p-2">
												<div className="d-flex align-items-center">
														<div className="image"> <img src={u.image} className="rounded" width="155" /> </div>
													<div className="ml-3 w-100">
															<h4 className="mb-0 mt-0 textLeft">{u.name} </h4>
				
															<div className="d-flex flex-column">
																	 <span className="number1">{u.orders} orders</span> </div>
													</div>
											 </div>
										  </div>
									</div>    
