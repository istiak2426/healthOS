    ...
    <Layout title="Your orders" description="" classNameNameName="container">
      <div className="col-md-6 col-lg-6 col-11 mx-auto my-auto search-box">
		...
		<div style={{margin:"10px"}}>
					...
					>All time({orderCount})</button>
					<button style={{margin:"16px"}}
					onClick={() => filteredItem("today")}
					>today({todayOrder.length})</button>
					<button style={{margin:"16px"}}
					onClick={() => filteredItem("yesterday")}
					>Yesterday({yesterdayOrder.length})</button>
...
{orders.filter((v)=> v.name.toLowerCase().includes(value.toLowerCase()) || v.id === parseInt(value) || v.phone === value)
.map((o)=>{
	return (
          ...
          <div className="row">
            ...
            <div className="col-md-7 col-11 mx-auto px-4 mt-2">
              ...
              <div className="row">
				...
				<button className="col-4 d-flex justify-content-center" >Details</button>
